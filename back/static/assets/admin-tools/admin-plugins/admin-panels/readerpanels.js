...
(function($, window, document, undefined) {
   ...
   // Plugin definition.
   $.fn.readerpanel = function(options) {
      ...
      var defaults = {
         ...
         onAction:function(){
            console.log('callback:', 'onAction');
         },
         onUpload:function(){
            console.log('callback:', 'onUpload');
         }
      };
      ...
      var restClient = options.restClient;
      var reader = options.reader;
      var panels = plugin.find('.panel');

      if(typeof restClient == 'string' || restClient instanceof String)
      ...
      // Reader's content changing handler
      $(reader).on('DOMSubtreeModified',function(){
   ...
   //          	if (This.length){
	  ...
	  //           	},2500);
         ...
         createControls: function(options) {
            ...
            var panelDownload = '<a href="#" class="panel-control-download"></a>';
            var panelUpload = '<a href="#" class="panel-control-upload"></a>';
            ...
            panels.each(function(i, e) {
               ...
               var downloader = This.attr('data-panel-download');
               var uploader = This.attr('data-panel-upload');
               ...
               if(!downloader){
                  var panelMenu = panelHeader.find('.panel-controls');
                  $(panelDownload).appendTo(panelMenu);
               }


               if(!uploader){
                  var panelMenu = panelHeader.find('.panel-controls');
                  $(panelUpload).appendTo(panelMenu);
               }
...
*/
         ...
         controlHandlers: function(e) {
            ...
            // download main content of article definition
            var panelDownloadArticle = function(){
               ...
               var toggleBox = function() {
                  ...
                  panelEditBox.slideToggle('fast', function() {
                     panel.toggleClass('panel-urlbox-open');

                     // Save settings to key if editbox is being closed
                     if (!panel.hasClass('panel-urlbox-open')) {
                        panel.addClass('panel-loader-active');
                        restClient.add('getarticle')
                              .read({'url':panelEditBox.children('input').val()})
                              .done(function(data){
                                 $(reader).html(unescape(data.cleaned_text).replace(/(?:\r\n|\r|\n)/g, '<br />'));
                        ...
                        console.log(panelEditBox.children('input').val());
               ...
               if (!panel.find('.panel-editbox').length) {
                  var editBox = '<div class="panel-editbox"><input type="text" class="form-control input-sm" placeholder="Url..."></div>';
            ...
            // Responses
            if ($(this).hasClass('panel-control-download')) {
               panelDownloadArticle();
            }
